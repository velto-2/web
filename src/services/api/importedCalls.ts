...
export const importedCallsApi = {
  upload: async (data: UploadCallRequest): Promise<any> => {
    ...
    if (data.files && data.files.length > 1) {
      ...
      if (data.externalCallId)
        formData.append("externalCallId", data.externalCallId);
      if (data.callDate) formData.append("callDate", data.callDate);
      if (data.agentId) formData.append("agentId", data.agentId);
      if (data.agentName) formData.append("agentName", data.agentName);
      if (data.campaignId) formData.append("campaignId", data.campaignId);
  ...
  getAnalytics: async (params?: {
    ...
    const response = await apiClient.get("/imported-calls/analytics", {
      params,
    });
    return response.data.data || response.data;
  },
  ...
  getBulkStatus: async (callIds: string[]): Promise<any> => {
    if (!callIds || callIds.length === 0) {
      throw new Error("At least one call ID is required");
    }
    const response = await apiClient.get("/imported-calls/bulk-status", {
  ...
  exportBulk: async (
    ...
    const response = await apiClient.post("/imported-calls/export-bulk", {
      callIds,
      format,
    });
    return response.data.data || response.data;
